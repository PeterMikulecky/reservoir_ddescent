...
3. **Sufficiency of Kind A (the empirical risk).** Is strength-tuning-within-fixed-support ENOUGH to build the memory/routing capability (D076/D082), or does that capability require STRUCTURE the birth topology lacks — which would force Kind B and break P? If the slow-current-capable topology is present at birth (D074) and only its strengths need tuning, Kind A suffices. If not, the whole redesign relocates rather than solves the problem. **This is the load-bearing unknown.**
